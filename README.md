...
- __Age__ should not be modified, and should thus measure age in years. You should rename the variable 'age'.
- __Sex__ should be coded as a 'dummy' coding `1` for females. you should rename the variable 'famale'
- __Employment relationship__ should not be modified, and should thus code `1` for employees, `2` for self-employed, and `3` for working for own family business. you should rename the variable 'employement_rel'
...
You should rename the variable 'marital_status'.
...
You should rename the variable 'sub_income'
...
You should rename the variable 'religious_att'.
...
Using the variables created in the previous section, predict religious attendance (the dependant variable) from age, sex, employment relationship, marital status and subjective income (the independant variables), for the entire ESS data sample.

In that model, include an interaction between sex and marital status, in order for your model to include coefficients for those predictors _as well as for their product_ : not only a linear model
...
Please [email me](mailto:kendrick.herzberg@sciencespo.fr) your group's answers as a single R script named `exercise-11-class-X-group-01.R`, where `01` is your group number and `X` is your class number, as indicated in the groups spreadsheet on Google Drive.


Please send me that email __along with your answer to Exercise 12, before the 12th of December at noon__.

You do _not_ need to also attach the data and/or the rest of the `exercise-11` folder -- the answers will be enough.

Use the email subject __"DSR: Answers to Exercises 11 and 12, Group 01 Class X"__, where `01` once again designates your group number and `X` is your class number. That information should also appear at the top of your script, in the code header.
